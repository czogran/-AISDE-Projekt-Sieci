...
Sciezka kruskal(vector<Krawendz> kraw, vector<Wezel> wezel) // trzeba bêdzie jeszcze zrobiæ dla skierowanego
{
	Sciezka sciezka(1);
	int test;
	test = 0;
	...
	{
		cout << sciezka.krawendzie[n].wezel_a<<"  "<<sciezka.krawendzie[n].wezel_b<<"\n";
		//rysuj_krawedz(wezel[sciezka.krawendzie[n].wezel_a - 1].x, wezel[sciezka.krawendzie[n].wezel_a - 1].y, wezel[sciezka.krawendzie[n].wezel_b - 1].x, wezel[sciezka.krawendzie[n].wezel_b - 1].y);
		//rysuj_krawedz(1, 2, 3, 4);
	}
	rysuj_graf_sciezka_kruskal(wezel, kraw, sciezka);
